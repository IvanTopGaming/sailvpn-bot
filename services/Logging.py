...
class LoggingService:
    ...
    async def on_user_request_keys(self, user: User):
        ...
        await self.app.tglogger.log(f"🔑 User {user.readable_name} ({user.uuid}) requested keys.")

    async def on_user_request_statistics(self, user: User):
        print("User requested statistics:", user.readable_name, user.uuid)
        await self.app.tglogger.log(f"📊 User {user.readable_name} ({user.uuid}) requested statistics.")
