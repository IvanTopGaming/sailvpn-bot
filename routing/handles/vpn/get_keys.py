...
async def get_keys(callback_query: CallbackQuery, app: App, vpn_user: User):
    tmp_message = await app.bot.send_message(
        callback_query.message.chat.id, "🔍 Поиск ключей...", parse_mode=None
    )
    ...
    if callback_query.data == "get_keys:main":
        ...
        if not additional_key:
            ...
            await callback_query.answer(
                "⛔ Дополнительный ключ не найден.", show_alert=True
            )
            return
    ...
    if not found_clients or len(found_clients) == 0:
        await callback_query.answer(
            "⛔ Ключи не найдены для данного пользователя.", show_alert=True
        )
        return
