...
async def get_statistics(callback_query, app: App, vpn_user: User):
    ...
    statistics = [app.user_repository.get_client_statistics(server, client_data) for server, client_data in client.items()]
    ...
    for statistics_data in statistics:
        ...
        answer += f"- Статус: {'🟢 Онлайн' if statistics_data.online else '🔴 Оффлайн'}\n"
        ...
        if len(statistics_data.ips) > 0:
            answer += f"- IP адреса в логе: \n```\n{'\n'.join(statistics_data.ips)}```\n"

        answer += f"\n"

    await tmp_message.delete()
    await callback_query.answer()
    await app.bot.send_message(callback_query.from_user.id, answer, parse_mode=ParseMode.MARKDOWN, reply_markup=get_delete_message_keyboard())
